...
useEffect(() => {
	...
	const LogoutUser =  () => {
	 ...
	 const sendMessage = () => {
    UseSendMessage({ socket:socket, user:user, messages:messages, message:message });
        ...
        <div className="w-screen flex">
          ...
          />
              ...
              filteredUsers.map(({ userId, user }) => (
               <SearchProfile user={user} userId={userId} fetchMessages={fetchMessages}/>
              ))
            )
			: <div className='text-center text-lg font-semibold mt-24'>No Other Users</div>
					}
				</div>
            </div>
        </div>
